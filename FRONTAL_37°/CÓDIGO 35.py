...
for i in range(1,participantes):
    ...
    while ind < len(lista_videos):  #CREA UNA LISTA CON LAS 3 PRUEBAS POR PART
        ...
        for video in elementos: #CREA UNA CARPETA POR PRUEBA
            ...
            angD_dtf.to_excel(os.path.join(new_path, 'Angulos Distal.xlsx'), index=False)      #CREA UN .CVS

            angM_dtf = angM_dtf[(angM_dtf != 0).all(axis=1)] 
            angM_dtf.to_excel(os.path.join(new_path, 'Angulos Medial.xlsx'), index=False)

            angP_dtf = angP_dtf[(angP_dtf != 0).all(axis=1)] 
            angP_dtf.to_excel(os.path.join(new_path, 'Angulos Proximal.xlsx'), index=False)
